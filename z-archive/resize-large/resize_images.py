#!/usr/bin/env python3
"""
Script to resize all JPG and PNG images in z-archive to 900x900 pixels.
For non-square images, crops the middle square portion.
PNG files are converted to JPEG format.
Saves resized images to the resize-large folder.
"""
...
def find_image_files(root_dir):
    """Find all JPG/JPEG and PNG files recursively in the directory, excluding resize folders."""
    image_files = []
    root_path = Path(root_dir)
    
    # Find all .jpg, .jpeg, and .png files recursively, excluding resize folders
    for ext in ['*.jpg', '*.jpeg', '*.JPG', '*.JPEG', '*.png', '*.PNG']:
        for file_path in root_path.rglob(ext):
            # Exclude files in resize-small and resize-large folders
            if 'resize-small' not in str(file_path) and 'resize-large' not in str(file_path):
                image_files.append(file_path)
    
    # Sort for consistent ordering
    return sorted(image_files)

def resize_image(input_path, output_path, size=(900, 900)):
